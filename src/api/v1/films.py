...
async def search_in_films(query: str,
                          page_size: int = Query(
                              50, ge=1, le=50,
                              description='Кол-во фильмов в выдаче (1-50)'),
                          page_number: int = Query(
                              1, ge=1, description='Номер страницы выдачи'),
    ...
    result = await film_service.films_search(query, page_size, page_number)
    if not result:
        raise HTTPException(
...
async def get_film_by_id(film_id: str,
                         film_service: FilmService = Depends(get_film_service)
                         ) -> FilmDetailSchema:
    '''
